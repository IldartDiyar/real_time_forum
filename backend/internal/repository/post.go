...
func (r *PostRepo) Create(post model.Post) error {
	...
	defer stmt.Close()
	res, err := stmt.Exec(post.UserID, post.Title, post.Description, post.CreateAt, post.UpdateAt)
	if err != nil {
		tx.Rollback()
		return err
	}
	...
	for _, categoryID := range post.CategoryID {
		_, err := tx.Exec(st, id, categoryID)
		if err != nil {
			tx.Rollback()
			return err
		}
...
func (r *PostRepo) Update(post model.Post) error {
	...
	_, err = tx.Exec(stmt, post.Title, post.Description, post.UpdateAt, post.ID)
	if err != nil {
		_ = tx.Rollback()
		return err
	}
	...
	if len(post.CategoryID) != 0 {
		...
		for _, category := range post.CategoryID {
			_, err = tx.Exec(stmt, post.ID, category)
			if err != nil {
				_ = tx.Rollback()
				return err
			}
